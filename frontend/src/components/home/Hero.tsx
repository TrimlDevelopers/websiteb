    ...
    <section className="hero-pattern overflow-hidden">
      <div className="site-container grid items-center gap-6 py-8 sm:gap-8 sm:py-12 lg:grid-cols-2 lg:gap-10 lg:py-16 xl:gap-12 xl:py-20">
        <div className="min-w-0 text-center lg:text-left">
          <span className="hero-enter hero-enter-1 inline-flex items-center rounded-full border border-brand-500/20 bg-brand-500/10 px-3 py-1 text-[11px] font-semibold tracking-wide text-brand-700 sm:px-4 sm:py-1.5 sm:text-xs">
            {company.heroBadge}
          </span>

          {/* LCP text stays fully opaque — no opacity animation on H1/subtitle/primary CTA */}
          <h1 className="mt-4 text-balance text-xl font-bold leading-[1.15] tracking-tight text-navy-900 sm:mt-5 sm:text-3xl md:text-4xl lg:text-5xl xl:text-[3.25rem]">
            {company.heroTitle}{' '}
            <span className="text-brand-700">{company.heroTitleAccent}</span>
          </h1>

          <p className="mt-4 text-sm leading-relaxed text-slate-600 sm:mt-5 sm:text-base lg:max-w-xl lg:text-lg xl:max-w-2xl">
          ...
          <div className="mt-6 flex flex-col gap-3 sm:mt-8 sm:flex-row sm:items-stretch">
            ...
            <Button href="/products" variant="outline" className="hero-enter hero-enter-4 w-full sm:w-auto">
